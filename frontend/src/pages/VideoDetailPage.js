...
export default function VideoDetailPage() {
  ...
  const scoreColor = scorePercent >= 80 ? 'var(--emerald)'
    : scorePercent >= 50 ? 'var(--amber)'
    : 'var(--rose)';
    ...
    <main className="main-content">
      <div className="container" style={{ maxWidth: 920 }}>
        ...
        <div className="card" style={{ padding: 0, overflow: 'hidden', marginBottom: 24 }}>
          <div style={{ display: 'grid', gridTemplateColumns: 'clamp(140px,220px,260px) 1fr' }}>
            {/* Thumbnail */}
            <div style={{ background: 'var(--bg-elevated)', flexShrink: 0, aspectRatio: '16/9', overflow: 'hidden' }}>
              <img
            ...
            {/* Info */}
            <div style={{ padding: 'clamp(14px,3vw,24px)', display: 'flex', flexDirection: 'column', justifyContent: 'space-between', gap: 12 }}>
              <div>
                <h1 style={{
                  fontFamily: 'var(--font-display)', fontWeight: 800,
                  fontSize: 'clamp(14px,2.2vw,20px)', lineHeight: 1.35,
                  marginBottom: 6,
                }}>{video.title}</h1>
                {video.channelName && (
                  <p style={{ fontSize: 13, color: 'var(--text-muted)', marginBottom: 0 }}>{video.channelName}</p>
                )}
              </div>
        ...
        <div key={activeTab} className="animate-fade-in">
          ...
          {activeTab === 'summary' && (
            <div className="card">
              ...
              <p style={{ fontSize: 15, lineHeight: 1.9, color: 'var(--text-secondary)', whiteSpace: 'pre-wrap' }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                  {(analysis.bulletPoints).map((point, i) => (
                    ...
                    >
                      ...
                      <p style={{ fontSize: 15, lineHeight: 1.7, color: 'var(--text-secondary)', margin: 0 }}>{point}</p>
                    </div>
                  ))}
                </div>
              )}
              ...
              {mcqs.length === 0 ? (
                ...
                <div key={qi} className="card">
                  <div style={{
                    ...
                    marginBottom: 16, lineHeight: 1.5, display: 'flex', gap: 10,
                  }}>
                    <span style={{
                    ...
                    {(mcq.options || []).map((opt, oi) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          <span style={{ fontSize: 14, color: textColor, fontWeight: isRevealed && (isCorrect || isSelected) ? 600 : 400, flex: 1 }}>
                            {opt}
                          </span>
                        </button>
                      );
